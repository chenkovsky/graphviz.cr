...
class GraphViz
  module Constants
    ...
    def self.get_attrs_for(x)
      list = {} of String => Symbol
      GENCS_ATTRS.each { |k, v|
        list[k] = v[:type] if x.match(v[:usedBy]) && !list.has_key?(k)
      }
      list
    end
    ...
    GRAPHSATTRS = GraphViz::Constants.get_attrs_for(/G|S|C/)

    # # Const: Node attributes
    NODESATTRS = GraphViz::Constants.get_attrs_for(/N/)

    # # Const: Edge attributes
    EDGESATTRS = GraphViz::Constants.get_attrs_for(/E/)
  end
end
